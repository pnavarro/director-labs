...
if hiera('step') >= 3 {
  ...
  if $glance_backend == 'file' and hiera('glance_file_pcmk_manage', false) {
    pacemaker::resource::filesystem { "glance-fs":
      device       => "${storage_vip}:/nfsshare/glance",
  ...
  if hiera('cinder_enable_nfs_backend', false) {
    ...
    cinder::backend::nfs { $cinder_nfs_backend:
      nfs_servers         => ["${storage_vip}:/nfsshare/cinder"],
...
if hiera('step') >= 4 {
  if $pacemaker_master {
    ...
    if hiera('cinder_enable_nfs_backend', false) {
      #lvm_device = hiera('nfs_lvm_device')
      #exec { "pvcreate $lvm_device":
      #  path        => ['/bin','/usr/bin','/sbin','/usr/sbin'],
      #  unless      => "pvdisplay | grep $lvm_device",
      #  refreshonly => true,
      #} ~>

      #exec { "vgcreate nfs_vg $lvm_device":
      #  path        => ['/bin','/usr/bin','/sbin','/usr/sbin'],
      #  unless      => "vgdisplay | grep nfs_vg",
      #  refreshonly => true,
      #} ~>

      #exec { "lvcreate -l100%FREE -n nfs_lv nfs_vg":
      #  path        => ['/bin','/usr/bin','/sbin','/usr/sbin'],
      #  unless      => "lvdisplay | grep nfs_lv",
      #  refreshonly => true,
      #} ~>

      #exec { "mkfs.ext4 /dev/nfs_vg/nfs_lv":
      #  path        => ['/bin','/usr/bin','/sbin','/usr/sbin'],
      #  refreshonly => true,
      #}

      pcmk_resource { "lvm-nfs":
        resource_type   => "LVM",
        resource_params => 'volgrpname="nfs_vg" exclusive=true',
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
        try_sleep       => 1,
        group_params    => 'nfsgroup',
      }

      pacemaker::resource::filesystem{ "nfsshare":
        device       => '/dev/nfs_vg/nfs_lv',
        directory    => '/nfsshare',
        fsoptions    => '',
        fstype       => 'ext4',
      ...
      pcmk_resource { "nfs-daemon":
        resource_type   => "nfsserver",
        resource_params => 'nfs_shared_infodir=/nfsshare/nfsinfo nfs_no_notify=true',
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
      ...
      pcmk_resource { "nfs-root":
        resource_type   => "exportfs",
        resource_params => "clientspec=${storage_vip}/255.255.255.0 options=rw,sync,no_root_squash directory=/nfsshare fsid=0",
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
      ...
      pcmk_resource { "nfs-glance":
        resource_type   => "exportfs",
        resource_params => "clientspec=${storage_vip}/255.255.255.0 options=rw,sync,no_root_squash directory=/nfsshare/glance fsid=1",
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
      ...
      pcmk_resource { "nfs-cinder":
        resource_type   => "exportfs",
        resource_params => "clientspec=${storage_vip}/255.255.255.0 options=rw,sync,no_root_squash directory=/nfsshare/cinder fsid=2",
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
      ...
      pcmk_resource { "nfs-nova":
        resource_type   => "exportfs",
        resource_params => "clientspec=${storage_vip}/255.255.255.0 options=rw,sync,no_root_squash directory=/nfsshare/nova fsid=3",
        meta_params     => '',
        op_params       => '',
        post_success_sleep => 0,
        tries           => 1,
      ...
      pacemaker::constraint::colocation { 'nfs_group-with-ip-storage_vip':
        source  => 'nfsgroup',
        target  => "ip-${storage_vip}",
        score   => 'INFINITY',
